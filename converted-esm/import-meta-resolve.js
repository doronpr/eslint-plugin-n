...
function read(jsonPath) {
    try {
        const string = import_node_fs.default.readFileSync(
            import_node_path2.default.toNamespacedPath(
                import_node_path2.default.join(
                    import_node_path2.default.dirname(jsonPath),
                    "package.json"
                )
            ),
            "utf8"
...
function getPackageConfig(path4, specifier, base) {
    ...
    try {
        ...
        throw new ERR_INVALID_PACKAGE_CONFIG(
            path4,
            (base ? `"${specifier}" from ` : "") +
                (0, import_node_url2.fileURLToPath)(base || specifier),
            exception.message
        )
    }
...
function getPackageScopeConfig(resolved) {
    ...
    while (true) {
        ...
        packageJsonUrl = new import_node_url2.URL(
            "../package.json",
            packageJsonUrl
        )
...
function emitInvalidSegmentDeprecation(
    ...
    import_node_process2.default.emitWarning(
        `Use of deprecated ${
            ...
            base
                ? ` imported from ${(0, import_node_url3.fileURLToPath)(base)}`
                : ""
        }.`,
        "DeprecationWarning",
        "DEP0166"
...
function emitLegacyIndexDeprecation(url, packageJsonUrl, base, main) {
    ...
    const pkgPath = (0, import_node_url3.fileURLToPath)(
        new import_node_url3.URL(".", packageJsonUrl)
    )
...
function legacyMainResolve(packageJsonUrl, packageConfig, base) {
    ...
    throw new ERR_MODULE_NOT_FOUND(
        (0, import_node_url3.fileURLToPath)(
            new import_node_url3.URL(".", packageJsonUrl)
        ),
...
function finalizeResolution(resolved, base, preserveSymlinks) {
    ...
    if (!preserveSymlinks) {
        ...
        resolved = (0, import_node_url3.pathToFileURL)(
            real + (filePath.endsWith(import_node_path3.default.sep) ? "/" : "")
        )
...
function throwInvalidSubpath(request, match, packageJsonUrl, internal, base) {
    const reason = `request is not a valid match in pattern "${match}" for the "${
        internal ? "imports" : "exports"
    }" resolution of ${(0, import_node_url3.fileURLToPath)(packageJsonUrl)}`
    throw new ERR_INVALID_MODULE_SPECIFIER(
        request,
        reason,
        base && (0, import_node_url3.fileURLToPath)(base)
...
function packageImportsResolve(name, base, conditions) {
    ...
    if (packageConfig.exists) {
        packageJsonUrl = (0, import_node_url3.pathToFileURL)(
...
function packageResolve(specifier, base, conditions) {
    ...
    const packageConfig = getPackageScopeConfig(base)
    if (packageConfig.exists) {
        const packageJsonUrl2 = (0, import_node_url3.pathToFileURL)(
            packageConfig.pjsonPath
        )
        if (
    ...
    do {
        ...
        if (!stat.isDirectory()) {
            ...
            packageJsonPath = (0, import_node_url3.fileURLToPath)(
                packageJsonUrl
            )
            continue
        }
        ...
        if (
            packageConfig2.exports !== void 0 &&
            packageConfig2.exports !== null
        ) {
            return packageExportsResolve(
                packageJsonUrl,
                packageSubpath,
...
function defaultResolve(specifier, context = {}) {
    ...
    ;(0, import_node_assert2.default)(
        typeof parentURL !== "undefined",
        "expected `parentURL` to be defined"
    )
    let parsedParentURL
    if (parentURL) {
        try {
    ...
    try {
        parsed = shouldBeTreatedAsRelativeOrAbsolutePath(specifier)
            ? new import_node_url3.URL(specifier, parsedParentURL)
            : new import_node_url3.URL(specifier)
        if (
            parsed.protocol === "data:" ||
    ...
    const url = moduleResolve(
        specifier,
        new import_node_url3.URL(parentURL),
        conditions,
        false
    )
